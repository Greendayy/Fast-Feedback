...
export async function updateSite(id, newValues) {
  const res = await supabase.from('sites').upsert({ id, newValues }).select();
  return res;
}
...
export async function updateFeedback(feedbackStatus) {
  console.log('updateFeedback', feedbackStatus);
  const res = await supabase
    .from('feedback')
    .update(feedbackStatus)
    .eq('status', feedbackStatus)
    .select();
  return res;
}
